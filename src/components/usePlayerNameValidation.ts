...
interface PlayerNameValidation {
  ...
  validateNames: (preferredPlayerId?: string) => boolean;
...
export function usePlayerNameValidation(
  ...
  const validateNames = (preferredPlayerId?: string): boolean => {
    const duplicateGroups = getDuplicateNameGroups(configs);
    setErrorIds((prev) => {
      const next = new Set<string>();
