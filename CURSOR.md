...
- **Online multiplayer (`AddMultiplayer`):** `OnlineLobby.tsx` + `useGameSession.ts` (client) + `worker/` (Cloudflare Worker + `GameRoom` DO). Dev: `npm run dev` + `npm run worker:dev`. `worker/.wrangler/` gitignored. **310 tests** (42 files: combat damage, earnings, ammo, shop, worker auth/catch-up, plus onlineSession / GameEngine.online / TurnManager / Terrain).
- **Test audit :** Réduction des tests redondants (counts projectiles, vent NaN, spies sprite). Ajout des règles combat/économie/munitions/shop/worker/IA/i18n/CSP. **310 tests**, 42 fichiers. — Grok 4.6 (xAI)
- **Multiplayer Sync Race Condition & React Doctor Fixes :** Résolution d'un bogue critique de synchronisation multijoueur où le nettoyage de connexion des slots supprimait le nouveau WebSocket de combat du dictionnaire des sockets actifs sur le serveur lors de la réception de l'événement de fermeture de l'ancien WebSocket du lobby. Validation par comparaison de référence (`this.sockets.get(slot) === ws`) ajoutée dans `handleSocketDisconnect` du Durable Object. Correction du nettoyage client au démontage dans `OnlineLobby.tsx` en copiant la référence de l'objet ref (`currentWsRef.current.close()`) pour éviter les alertes de cycle de vie. Ajout de la clé de traduction `link_instructions` pour passer le typecheck TypeScript strict de l'i18n. Score de santé `react-doctor` à 84/100, l'ensemble des 158 tests passent au vert. — Antigravity (Gemini 3.5 Flash (High))
- **Durable Object State Persistence :** Implémentation de la persistance d'état transactionnelle pour le Durable Object `GameRoom` via les API `storage.get` / `storage.put`. Restauration asynchrone lors des cold starts (`ctx.blockConcurrencyWhile`), et conversion des handlers WS principaux, de l'auto-démarrage et de l'exécution des tirs en méthodes asynchrones afin de persister de manière sécurisée les changements d'état. — Antigravity (Gemini 3.5 Flash (High))
- **Cloudflare Worker TypeScript & Type checking :** Intégration du typechecking statique pour le dossier du Worker (`worker/`) via un fichier de configuration `worker/tsconfig.json` dédié référencé dans le `tsconfig.json` de la racine. Résolution de toutes les erreurs de typage TypeScript dans le Durable Object et l'index du worker (utilisation des types globaux `DurableObjectNamespace` / `DurableObjectState` au lieu d'imports de plateforme, typage correct du roster et signature stricte de retour `Color` pour `assignColor`). — Antigravity (Gemini 3.5 Flash (High))
- **Migration du modèle d'agent (Gemini 3.6 Flash) :** Installation de la compétence `gemini-interactions-api` et mise à jour de la configuration de l'agent de maintenance vers `Gemini 3.6 Flash` afin de prendre en charge les workflows agentiques et multimodaux avancés avec une meilleure efficacité de jetons. Validation réussie de la suite complète de 231 tests. — Antigravity (Gemini 3.6 Flash)
- **Production deploy option B :** `VITE_API_BASE` → workers.dev (`onlineApi.ts`), CSP `*.workers.dev`. 158 tests. Script deploy local gitignored. — Grok 4.3 (xAI)
