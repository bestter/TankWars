...
- Step 4, 5, 6 & 7 Polish (recent): 
  ...
  - **Game Version Bump:** Bumped game version to `0.3.3` in `package.json` and `package-lock.json`.
  - **Version Display on Main Menu:** Imported game version from `package.json` and added it to the footer of `MainMenu.tsx` beside the license notice (e.g. `v0.3.3`).
