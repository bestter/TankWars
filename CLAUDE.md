...
- **Online:** In `main` (not a feature branch). Cloudflare Worker + `GameRoom` Durable Object (`worker/`) owns turn order, FIRE validation/ammo consumption, transactional shop sessions with composite idempotency keys, reward/balance application, round end, and round-scoped reconnect history (`shotHistory` cleared on shop completion). Clients launch local Canvas physics only after the correlated authoritative `SHOT`; client FIRE on AI turns is rejected with `NOT_YOUR_TURN`; rejection notices display via a non-blocking toast alert (`.fire-rejection-toast`). A restored `SHOP` that has not received its first `SHOP_STATE` retries `SHOP_ENTER`; the Worker creates or resumes the same session without repeating AI purchases. Client: `OnlineLobby.tsx` + `useOnlineLobby.ts` + create/waiting views; combat in `useGameSession.ts` / `onlineSession.ts` with strict session storage validation. Shared files: `src/game/online/turnOrder.ts` and strict `protocol.ts` (`ONLINE_PROTOCOL_VERSION` 1 on `GAME_START`/`REQUEST_GAME_START`; mismatch → `PROTOCOL_MISMATCH` + close `4402`, client overlay, Pages then Worker). Shot replay is `authoritativeShotQueue.ts`; live shop transitions during replay go through `DeferredTransitionBuffer`. The first connected human is reward authority; persistent failover promotes the next original human. Full authoritative terrain/damage simulation is planned. `GAME_START` includes `materials` only when the server array matches `heights` length; `loadHeights` resets to `DIRT` otherwise.
