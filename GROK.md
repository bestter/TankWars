...
- After edits: `npm run lint && npm run build && npm run test` (**745 tests**, 72 files).
...
- **Shared strict protocol:** `src/game/online/protocol.ts`; `ONLINE_PROTOCOL_VERSION` (1) on `GAME_START`/`REQUEST_GAME_START`; mismatch → `PROTOCOL_MISMATCH` + close `4402`. `actionId` and composite keys make FIRE/shop intentions idempotent, and reward authority is the first connected human with persistent ordered failover.
- **Deploy:** Pages first, then `npm run worker:deploy` (separate from Cloudflare Pages)
...
- Online MVP: local physics launched from authoritative `SHOT` echoes; the server owns turn order, ammo consumption, shop transactions (composite idempotency keys), and reward application. `GameRoom` persists authority epoch/order, active shot/round-scoped history, shop epochs/actions, balances, deaths, and terminal results; duplicate intentions cannot apply twice. A restored `SHOP` without its first `SHOP_STATE` retries `SHOP_ENTER`; create/resume handling preserves the shop epoch and applies AI purchases once. Client FIRE during an AI turn is rejected with `NOT_YOUR_TURN`; rejection notices use a non-blocking toast alert (`.fire-rejection-toast`). Shot replay lives in `authoritativeShotQueue.ts`; a live `SHOP_STATE`/`SHOP_FINISH` pair during replay is ordered by `DeferredTransitionBuffer`. Mixed protocol versions surface an overlay and close `4402`. Full authoritative terrain/damage sim is still planned. `GAME_START` sends `materials` only when the server array matches `heights` length. `loadHeights` resets every column to `DIRT` if materials are omitted or mismatched.
