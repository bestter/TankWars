...
- After edits: `npm run lint && npm run build && npm run test` (**357 tests**, 48 files).
...
- AI v1 is naive and must stay that way. v2–v4 aim through `fallibleAim.ts` (see AGENTS.md table). Warmup: `roundSkill` starts at 10% on manche 1 and reaches full spec at manche 5; Simple is alcoholic with P = `1 - skill`.
