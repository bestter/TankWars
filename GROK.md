...
- After edits: `npm run lint && npm run build && npm run test` (**378 tests**, 48 files).
...
- Terrain: multi-octaves procedural heightmap with bumps/creux. Materials: `DIRT` (normal), `ROCK` (indestructible stone, +50% blast damage, radius unaffected), `SOFT` (2.5x destruction multiplier).
