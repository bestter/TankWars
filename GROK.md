...
- After edits: `npm run lint && npm run build && npm run test` (**589 tests**, 66 files).
...
- Local AI names use the localized short profile label (`Simple`, `OK`, `Sniper`, `Expert`). Duplicate suffixes count every other configured player with that profile, regardless of slot order (`Simple`, `Simple-1`, `Simple-2`). Only the newly selected slot is renamed; names remain editable and do not change with later language switches.
