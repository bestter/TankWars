...
- **Performance (v0.4.1):** Terrain partial dirty-band offscreen redraw; HUD updates throttled to ~15 Hz with `React.memo` on `GameHUD`/`MobileControls`; celebration fireworks at 60 Hz with in-place particle compaction and 250-particle cap
...
- **Test Suite (v0.4.1)**: **112 unit tests** across 11 files (Vitest), including ballistic simulation, AI profile dispatcher, terrain dirty-band redraw, HUD throttle, and fireworks optimization coverage.
