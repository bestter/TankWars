...
## 2024-05-18 - [Infinity Injection Vulnerability]
**Vulnerability:** In `worker/src/game-room.ts` and `src/components/useGameSession.ts`, untrusted numeric inputs (like player money, health, and shot slots) were validated using `!Number.isNaN(value)`. This allowed attackers to pass `Infinity` or `-Infinity`, bypassing validation and potentially crashing the physics engine or granting infinite resources (e.g., `Math.max(0, Infinity)` returns `Infinity`).
**Learning:** `Number.isNaN()` only checks if a value is strictly `NaN`. It does not protect against `Infinity` or `-Infinity`, which are technically numbers in JavaScript but can cause severe logic bugs or Denial of Service when used in game state or array indices.
**Prevention:** When validating untrusted continuous numeric inputs (e.g., game stats, coordinates, physics parameters) from external sources, strictly use `Number.isFinite()` rather than just `typeof x === 'number'` or `!Number.isNaN()`.
